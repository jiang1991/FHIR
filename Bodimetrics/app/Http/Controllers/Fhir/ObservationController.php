...
class ObservationController extends Controller
{
  ...
  */
  function ObservationCreate()
  {
    ...
    $patient_id = $observationData->subject->reference; //这里其实是PatientId
    ...
    for ($i=0; $i < $com_num; $i++) {
      ...
      if (array_key_exists("valueString", $component[$i])) {
        $observation_component->valueString = $component[$i]->valueString;
      } elseif (array_key_exists("valueAttachment", $component[$i])) {
        $observation_component->valueAttachment = $component[$i]->valueAttachment;
      } else {
    ...
    // CareSpan Notification
    if ($user->is_carespan) {
      $notice["user_id"] = $user_id;
      $notice["resource_type"] = 'observation';
      $notice["patient_id"] = $patient_id;
      $notice["observation_id"] = $observation_id;
    ...
    return response($response)
      ->header('Content-Type', 'application/json+fhir')
      ->header('Location', 'http://cloud.bodimetrics.com/observation/' . $observation_id);
  }
  ...
  */
  function ObservationRead($observation_id)
  {
    ...
    for ($i=0; $i < $com_num; $i++) {
      if ($Qcomponent[$i]['code_display'] == 'Rate Pressure Product') {
        # code...
      } else {
        ...
        if (empty($Qcomponent[$i]['valueString'])) {
